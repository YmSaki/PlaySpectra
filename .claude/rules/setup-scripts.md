...
5. **ランタイム能力に依存する E2E は「probe 先行 → 観測された拒否のみ SKIP 変換」で組む。**
   実装前にそのランタイムが能力(MSAA/HDR/TYPELESS 等)を受理するか最小 probe で実測し、テスト側は
   「rc!=0 かつ 該当 env 有効 かつ ログに実際の拒否痕跡」が揃ったときだけ rc=0 の明示 SKIP へ変換する。
   静的な「ランタイム×能力」対応表をテストに埋め込まない — 将来ランタイムが受理すれば無改修で
   フルアサーションに昇格する(アサーション自体は env ガードで既定テストに影響させない)。
   — 出典: journal L60/L61/L64/L70 (R08 で設計、R09/R17 が無改修で再利用)

6. **パッチの冪等マーカーは「挿入テキストに実在する部分文字列」を機械確認してから使う。**
   `sub(marker, old, new)` 方式で marker が new に含まれないと、初回は成功するのに再実行で anchor 消費済み
   +marker 不一致 → assert 死(整列用スペースの差が典型原因)。受入基準は
   **「setup スクリプト2連続実行で rc=0 かつ patched 出力0行」** — 全 setup パッチ変更で必ず回す。
   — 出典: journal L71 (R17 review が read-only probe で検出)
