...
| **PlaySpectra Server**: 高水準命令＋補間 | ✅ `move_head`/`look`/`walk_forward`/`strafe`/`trigger`/`press`/`set_input`/`move_controller`/`reset`/`get_state`。**live Windows で --verify 9/9（HMD操作+auto-wait）+ controller_ops 7/7（move_controller grip+aim・set_input squeeze/button・reset）実測** | `tools/playspectra_server.py`（`--verify` 6/6、`controller_ops` 7/7、Windows 2026-07-22） |
...
| **MCP サーバー（現行・Python）** | ✅ FastMCP が Server をラップ（operate→:52702 / capture→:52700）。**live Windows Monado + hello_xr 相手に実 MCP クライアントで 7/7**（tools一覧/move_head/look/get_state/screenshot が実画像/run_scenario assert/reset） | `tools/playspectra_mcp.py`、`scripts/run_mcp_verify_monado.sh`（要 `pip install mcp`・**venv 推奨**、2026-07-22） |
