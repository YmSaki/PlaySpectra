 ...
 *         device core. The control channel WRITES it; the runtime adapter's virtual
 *         devices (hmd, left/right controllers) READ it. Decoupling via this shared
 *         struct avoids device-lifetime hazards: the control thread never touches
 *         runtime device objects directly, so the runtime's device destroy order
 *         cannot race a live control thread.
 *
 *         Pose はランタイム中立の playspectra_pose(playspectra_proto.h)で保持する。
 *         ランタイム固有型(Monado の xrt_space_relation 等)への変換は各 Adapter の
 *         殻(読み出し側)が行う。
 */
...
/*!
 * Shared device state. Refcounted: created by the adapter, referenced by each device
...
struct playspectra_ctrl
{
	...
	float thumbstick_x; // /input/thumbstick (x)
	float thumbstick_y; // /input/thumbstick (y)
...
/*!
 * 現在の head + 両コントローラ状態を「起動時スナップショット」として保存する。Adapter が
