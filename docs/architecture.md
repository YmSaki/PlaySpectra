...
SteamVR role mapping is class-specific. Controller hand roles map to `Prop_ControllerRoleHint_Int32`. Generic tracker body roles are normally user assignments in Manage Trackers; any future driver-side synchronization uses `IVRSettings` as a separate, opt-in contract rather than a generic role property.
...
The core keeps controller grip and aim as independent poses. The SteamVR MVP cannot preserve arbitrary per-frame relative motion between them: it maps grip to the tracked-device root and exposes aim as a fixed/calibrated render-model pose offset. That degradation must be capability-visible rather than silently presented as full contract support.
