...
| SteamVR Adapter | **Planned** | `driver/` contains an earlier controller-only skeleton. The target adapter separately verifies external-driver registration, HMD-present preflight, and physical-HMD-free runtime startup; embeds the runtime-neutral device core; exposes a virtual HMD and left/right controllers with owned bindings; returns haptics to core observers; and passes a SteamVR + native OpenXR operate -> render -> observe E2E. Native OpenVR operation and rendered-frame observation are reported separately. Source: docs/steamvr-adapter.md. |
...
- Total: 512 on Windows or 503 on non-Windows.
