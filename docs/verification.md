...
| SteamVR Adapter | **Planned** | `driver/` contains an earlier controller-only skeleton. The target adapter must pass headless HMD discovery/startup, embed the runtime-neutral device core, expose a virtual HMD and left/right controllers with owned bindings, return haptics to core observers, and pass a SteamVR + native OpenXR operate -> render -> observe E2E. Native OpenVR operation and rendered-frame observation are reported separately. Source: docs/steamvr-adapter.md. |
