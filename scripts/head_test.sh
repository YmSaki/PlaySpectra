...
# channel (head_client.mjs), and capture screenshots. POSE_X/POSE_Y/POSE_Z/HOLD_MS pass through.
set -u
...
echo "Launching hello_xr -g ${GFX}; head yaw=${HEAD_YAW_DEG:-0} pos=(${HEAD_X:-0},${HEAD_Y:-0},${HEAD_Z:-0})"
...
node "${PLAYSPECTRA_ROOT}/scripts/head_client.mjs"
...
grep -iE "LocateViews|head|ReferenceSpace" "$PLAYSPECTRA_LOG" | tail -12
