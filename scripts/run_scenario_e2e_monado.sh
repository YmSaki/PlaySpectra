...
# (:52700) via lib_monado_stack.sh, then has playspectra run tools/scenarios/capture_assert_demo.json:
# capture a reference via the layer, assert a no-op leaves it stable, drive look+move_head via the
# Monado adapter, assert the captured image changed. All assert logic lives in the Go control plane --
# this script only stands up the live stack. Machine verdict = playspectra rc (0 iff every assert passes).
...
if mstack_up "$GFX" "$SECS"; then
  sleep 1
  echo "=== flagship loop: playspectra runs $(basename "$SCENARIO") (operate :52702 -> observe :52700 -> assert) ==="
  ps_run run "$SCENARIO" --port 52702 --capture-port 52700; RC=$?
else
