...
#   raw client -> vr_input(squeeze=1.0) -> vr_agent layer -> XR_EXT_conformance_automation
...
node "${VRMCP_ROOT}/scripts/inject_client.mjs" 6000
...
echo
echo "=== vr_agent layer log (key lines) ==="
grep -iE "control.channel|conformance|instance created|session created|setInputDevice|buzzed|injecting|runtime:" "$VR_AGENT_LOG" || echo "(no matching lines)"

echo
echo "=== haptic proof (app reacted to injected grab) ==="
if grep -qi "buzzed the controller" "$VR_AGENT_LOG"; then
